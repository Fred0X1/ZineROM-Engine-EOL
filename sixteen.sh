#!/bin/bash
set -e # إجبار السكريبت على الموت فوراً لو أي أمر داخلي فشل
...
echo "📱 [1/7] جاري إعداد متغيرات الأجهزة والمسارات..."
# Device info
...
echo "📦 [2/7] جاري استخراج ملف Super.img وتفكيك الفيرموير..."
EXTRACT_SUPER_IMG "$FIRM_DIR/$TARGET_DEVICE"
EXTRACT_FIRMWARE_IMG "$FIRM_DIR/$TARGET_DEVICE" "all"

echo "🧹 [3/7] جاري تنظيف الروم (Debloat) وتعديل ملفات الـ OMC..."
DECODE_OMC "$FIRM_DIR/$TARGET_DEVICE"
DEBLOAT "$FIRM_DIR/$TARGET_DEVICE"

echo "🔧 [4/7] جاري تطبيق رقع الحماية (SELinux) وإضافة تطبيقات الفلاج شيب..."
...
echo "⚙️ [5/7] جاري بدء عمليات الـ Apktool وتفكيك الـ Jars (قد تستغرق وقتًا)..."
...
echo "🛠️ [6/7] جاري تعديل الـ Jars (SSRM / Secure Folder / Private Share)..."
...
echo "💾 [7/7] جاري بناء وتجميع ملفات الـ .img النهائية (BUILD_IMG)..."
BUILD_IMG "$FIRM_DIR/$TARGET_DEVICE" "all" "$OUTPUT_FILESYSTEM" "$OUT_DIR"

echo "✅ تم الانتهاء من سكريبت البناء بنجاح!"
